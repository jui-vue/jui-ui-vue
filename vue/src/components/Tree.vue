...
import { parseIndex, getParentIndex, getNextIndex } from "../utils/treeIndex.js"
...
function update(indexOrList, data) {
    if (arguments.length === 2) {
        ...
        for (const row of dataList) {
            const pIndex = getParentIndex(row.index)
...
function move(index, targetIndex) {
    ...
    const keys = parseIndex(targetIndex)
...
function dragDropOnNode(node, e) {
    ...
    if (props.dragChild !== false && dragStart.value !== node.index) {
        ...
        const endIndex = lastChild ? getNextIndex(lastChild.index) : node.index + ".0"
