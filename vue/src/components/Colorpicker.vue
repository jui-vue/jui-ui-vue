<script setup>
// 원본(colorpicker.js)은 jui.include("util.color")(juijs 코어의 색상 유틸, 이 리포에
// vendoring 안 됨)에 의존한다 — 같은 시그니처로 ../utils/color.js에 재구현해서 사용한다.
...
import { parseColor, formatColor, hsvToRgb, rgbToHsv, colorScale } from "../utils/color.js"
...
function checkHueColor(p) {
    ...
    if (startColor && endColor) {
        const scale = colorScale().domain(startColor.rgb, endColor.rgb)
...
function onHexKeyup(e) {
    if (e.which === 13) {
        ...
        if (code.charAt(0) === "#" && (code.length === 7 || code.length === 4)) {
            initColor(code, "hex")
            emit("enter", code, parseColor(code))
        }
    }
}
function onRgbKeyup() {
